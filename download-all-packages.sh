...
echo "在当前机器上执行:"
echo ""
echo -e "  ${GREEN}tar -czf nvidia-full-$(date +%Y%m%d).tar.gz packages/ install-all-offline.sh${NC}"
echo ""

echo -e "${CYAN}步骤 2: 传输到目标机器${NC}"
echo "使用 SCP 或其他方式传输:"
echo ""
echo -e "  ${GREEN}scp nvidia-full-$(date +%Y%m%d).tar.gz user@target-host:/tmp/${NC}"
echo ""

echo -e "${CYAN}步骤 3: 在目标机器上解压并安装${NC}"
echo ""
echo -e "  ${GREEN}cd /tmp${NC}"
echo -e "  ${GREEN}tar -xzf nvidia-full-$(date +%Y%m%d).tar.gz${NC}"
