...
echo "在当前机器上执行:"
echo ""
echo -e "  ${GREEN}tar -czf nvidia-driver-cuda-${DATE_STAMP}.tar.gz packages/ install-driver-cuda.sh${NC}"
echo ""

echo -e "${CYAN}步骤 2: 传输到目标机器${NC}"
echo "使用 SCP 或其他方式传输:"
echo ""
echo -e "  ${GREEN}scp nvidia-driver-cuda-${DATE_STAMP}.tar.gz user@target-host:/tmp/${NC}"
echo ""

echo -e "${CYAN}步骤 3: 在目标机器上解压并安装${NC}"
echo ""
echo -e "  ${GREEN}cd /tmp${NC}"
echo -e "  ${GREEN}tar -xzf nvidia-driver-cuda-${DATE_STAMP}.tar.gz${NC}"
